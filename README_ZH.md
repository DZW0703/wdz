...
WDZ 可能会提供额外引导输入来帮助 agent 对话，但 agent 的可见输出应尽量精简，避免不必要的中间文字、原始扫描结果、过程叙述、重复结果和草稿片段，除非 WDZ 明确要求。
...
WDZ 的电脑上有 Reasonix 和 Claude Code，二者都可使用 DeepSeek V4 Pro。对于低风险、耗 token 的委派任务，优先使用 TaskPorter -> Reasonix/DS Pro。

如果 Codex 看不到 `worker_*` 工具，应先检查 `C:\Users\Administrator\.codex\config.toml` 是否注册了 `taskporter-mcp`；如果当前会话仍没有 MCP 工具，则使用：
